...
<div id="modal-{{ $movie->id }}" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative p-4 w-full max-w-2xl max-h-full">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                <img class="rounded-t-lg" src="{{ asset($movie->banner) }}" alt="" />
            <div class="p-5 bg-black ">
                    <h5 class="mb-2 text-2xl font-bold tracking-tight text-white-900 dark:text-white">{{ ($movie->title) }}</h5>
                <p class="mb-3 font-normal text-white-700 dark:text-gray-400">{{ ($movie->description) }}</p>
                
                <form class="max-w-sm mx-auto" action="{{ route('browse.rate', $movie) }}" method="POST">
                    @csrf
                    <div class="mb-4">
